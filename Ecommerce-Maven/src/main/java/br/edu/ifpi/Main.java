            ...
            switch (opcao) {
                case 1:
                    ...
                    if (!url.startsWith("http://") && !url.startsWith("https://")) {
                        ...
                        urlInvalida = true; // Usuário confirmou que quer continuar com URL inválida
                    }
                    
                    // Usando Factory Method - Criação robusta e validada
                    try {
                        br.edu.ifpi.Factory.ProdutoDigitalFactory factory = 
                            (br.edu.ifpi.Factory.ProdutoDigitalFactory) br.edu.ifpi.Factory.ProdutoFactoryProvider.criarFactoryProdutoDigital(nome, preco, desc, url);
                        
                        // Se URL é inválida mas usuário confirmou, permite na factory
                        if (urlInvalida) {
                            factory.permitirUrlInvalida();
                        }
                        
                        br.edu.ifpi.Model.ProdutoDigital pd = (br.edu.ifpi.Model.ProdutoDigital) factory.criarProdutoCompleto();
                        dao.salvar(pd);
        ...
        do {
            ...
            switch (opcao) {
                case 1:
                    ...
                    // Usando Factory Method - Criação robusta e validada
                    try {
                        br.edu.ifpi.Factory.ProdutoFactory factory = 
                            br.edu.ifpi.Factory.ProdutoFactoryProvider.criarFactoryProdutoFisico(nome, preco, desc, peso, estoque);
                        br.edu.ifpi.Model.ProdutoFisico pf = (br.edu.ifpi.Model.ProdutoFisico) factory.criarProdutoCompleto();
                        dao.salvar(pf);
                ...
                switch (opcao) {
                    case 1: // CADASTRAR CLIENTE
                        try {
                            ...
                            // Usando Factory Method - Criação robusta e validada (inclui validação de endereço)
                            br.edu.ifpi.Model.Cliente cliente = br.edu.ifpi.Factory.ClienteFactory.criarCliente(nome, email, senha, cpfFormatado, endereco);
                            dao.salvar(cliente);
            ...
            switch (opcao) {
                ...
                case 5: // ADICIONAR PAGAMENTO
                    ...
                    if (tipoPagamento == 1) {
                        ...
                        // Usando Factory Method - Criação robusta e validada
                        try {
                            br.edu.ifpi.Factory.PagamentoFactory factory = 
                                br.edu.ifpi.Factory.PagamentoFactoryProvider.criarFactoryBoleto(codigoBoleto);
                            br.edu.ifpi.Model.Boleto boleto = (br.edu.ifpi.Model.Boleto) factory.criarPagamentoCompleto();
                            pedidoPagamento.setPagamento(boleto);
                            dao.atualizar(pedidoPagamento);
                            System.out.println("✅ Pagamento (Boleto) adicionado ao pedido!");
                        } catch (IllegalArgumentException e) {
                            System.out.println("❌ Erro na criação do boleto: " + e.getMessage());
                        }
                    } else {
                        System.out.println("❌ Opção inválida!");
                    }
            ...
            switch (opcao) {
                case 1: // CRIAR BOLETO PARA PEDIDO
                    try {
                        ...
                        // Passo 9: Criar o boleto com valor usando Factory Method
                        try {
                            // Converter data de YYYY-MM-DD para DD/MM/YYYY antes de passar para factory
                            String vencimentoFormatado = formatarData(vencimento);
                            br.edu.ifpi.Factory.PagamentoFactory factory = 
                                br.edu.ifpi.Factory.PagamentoFactoryProvider.criarFactoryBoleto(codigoBoleto, vencimentoFormatado, pedido.getTotal());
                            br.edu.ifpi.Model.Boleto boleto = (br.edu.ifpi.Model.Boleto) factory.criarPagamentoCompleto();
